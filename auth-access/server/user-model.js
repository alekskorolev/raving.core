...
/*    save: function (req) {
    ...
    register: function (req) {
      ...
      this.Model.create(regdata, function (err, user) {
        if (err || !user) {
          return req.respond({
            success: !err,
            err: err ? err.message : 'USER_REGISTRATION_ERROR',
            user: null
          });
        }
    ...
    clearBeforeSave: function (data) {
      var clear = {
        ...
        _password: helper.getProperty(data, '_password.isNew') ? {
          isNew: true,
          value: data._password.value,
          revalue: data._password.revalue
        } : undefined
      };
      return clear;
    },
    login: function (req) {
      ...
      if (!(data.login && data.password)) {
        return req.respond({
          success: false,
          err: 'LOGIN_AND_PASSWORD_MUST_HAVE',
          user: null
        });
      }
      this.Model.findOne({
        ...
        .exec(function (err, user) {
          if (user && !err) {
            ...
            req.respond({
              success: false,
              err: 'USER_NOT_FOUND_OR_DATABASE_ERROR',
              user: null
            });
          }
        });
      ...
      if (access && access instanceof Array && access.length > 0) {
        accessModel.fetch({
          ...
          respond: function (result) {
            var next = [], key, acc, _acc, id;
            if (result.collection.length > 0) {
              // add root object list in calculate array
              for (key in result.collection) {
                if (result.collection.hasOwnProperty(key)) {
                  acc = result.collection[key];
                  ...
                  if (acc.child && acc.child instanceof Array && acc.child.length > 0) {
                    ...
                    for (key in acc.child) {
                      if (acc.child.hasOwnProperty(key)) {
                        _acc = acc.child[key];
                        ...
                        if (_acc.child && _acc.child instanceof Array && _acc.child.length > 0) {
                          ...
                          for (key in _acc.child) {
                            if (_acc.child.hasOwnProperty(key)) {
                              id = _acc.child[key]._id;
  ...
  userModel = modelFactory(models.users);
  ...
  userModel.Schema.pre('save', function (next) {
    ...
    if (pass) {
      if (!(pass.value && pass.value.length)) {
        return next(new Error('PASSWORD_AT_LEAST_6_CHARS'));
      }
      if (pass.value !== pass.revalue) {
        return next(new Error('CONFIRM_PASSWORD_INCORRECT'));
      }
  ...
  app.io.route('auth:register', function (req) {
    userModel.register(req);
  });
  app.io.route('auth:login', function (req) {
    userModel.login(req);
  });
  app.io.route('auth:logout', function (req) {
    userModel.logout(req);
  });
  app.io.route('auth:activate', function (req) {
    userModel.activate(req);
  });
  app.io.route('auth:recovery', function (req) {
    userModel.recovery(req);
  });
  app.io.route('auth:check', function (req) {
    userModel.checkauth(req);
  });
};